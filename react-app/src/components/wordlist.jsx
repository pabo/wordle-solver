...
const CandidateWord = ({word, wordSelectedHandler}) => {
	...
	return <div className="candidate-word" data-word={word} onClick={handleClick}>
		{word}
	</div>
}
...
export const WordList = ({list, wordSelectedHandler}) => {
	return <div className="wordlist">
		...
		<div className="candidate-words">
			{list.map((word, index) => <CandidateWord key={index} word={word} wordSelectedHandler={wordSelectedHandler}/>)}
		</div>
	</div>;
}
