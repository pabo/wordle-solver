...
onmessage = (message) => {
  const {
    data: { mode, wordsToScore, wordsToScoreAgainst },
  } = message;
  ...
  wordsToScore.forEach((word) => {
    ...
    if (cache.has(key)) {
      ...
      // TODO: I think the size of the cache blows up. need to figure it out.
  ...
  console.log(
    `ending worker for mode ${mode} with ${cacheHits}/${
      cacheHits + cacheMisses
    } cache hits (${100* cacheHits / (cacheHits + cacheMisses)}%)`
  );
